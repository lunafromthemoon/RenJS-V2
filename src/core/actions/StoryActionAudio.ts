...
export default class StoryActionAudio extends StoryAction {
	...
	protected params: {actor:string, looped: boolean, transition: string, actorType, fromSeconds: number}
    ...
    execute(): void {
        if (this.action=='play'){
            if (this.params.actorType === 'music') {
                this.game.managers.audio.play(this.params.actor, 'bgm', this.params.looped, this.params.fromSeconds, this.params.transition);
            } else {
