...
export class StoryActionShow extends StoryAction {
  actor: string;
  actorType: string;
  transition: string;
  ...
  constructor(
    ...
    if (!this.transition) {
      this.transition =
        this.game.storyConfig.transitions.defaults[this.actorType];
    }
    ...
    if (this.actorType === 'cgs') {
      this.properties.layer = this.parseParameter('BEHIND')
        ? 'behindCharactersSprites'
        : 'cgsSprites';
    }
    ...
    if (this.actorType === 'characters') {
      ...
      if (
        this.game.managers.story.reservedWords.includes(this.properties.look)
      ) {
  ...
  execute(): void {
    const manager: RJSSpriteManagerInterface =
      this.game.managers.story.getManagerByActorType(this.actorType);
    const transitioning: Promise<any> = manager.show(
      this.actor,
      this.transition,
      this.properties
    );
...
export default StoryActionShow;
