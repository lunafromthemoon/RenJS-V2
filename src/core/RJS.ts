...
import { Ambient, Effects, Transition } from '../screen-effects/';
...
export class RJS extends Game {
  ...
  async initStory(): Promise<any> {
    if (this.storyConfig.backgroundColor) {
      const color =
        typeof this.storyConfig.backgroundColor === 'string'
          ? this.storyConfig.backgroundColor
          : Phaser.Color.fromRGBA(this.storyConfig.backgroundColor).rgba;
      this.canvas.style.backgroundColor = color;
    }

    this.userPreferences = new UserPreferences(
      this,
      this.storyConfig.userPreferences
    );
    ...
    if (!this.setup.lazyloading) {
      ...
      const audioList = Object.keys(this.setup.music).concat(
        Object.keys(this.setup.sfx)
      );
  ...
  async unpause(): Promise<any> {
    ...
    if (
      ...
      this.managers.logic.currentChoices.length === 0
    ) {
      this.resolveAction();
    }
  }
  ...
  async save(slot?): Promise<any> {
    ...
    if (this.xShots && this.xShots.length) {
      ...
      localStorage.setItem(
        'RenJSThumbnail' + this.config.name + slot,
        thumbnail
      );
    }
  ...
  async loadSlot(slot): Promise<void> {
    ...
    this.managers.story.currentScene = this.control.execStack.getActions(
      this.story
    );
  ...
  waitForClick(callback?): void {
    ...
    if (this.control.skipping || this.control.auto) {
      let timeout = this.control.skipping
        ? this.storyConfig.skiptime
        : this.storyConfig.autotime;
      if (this.control.auto) {
        // max autospeed == 300
  ...
  waitTimeout(time, callback?): void {
    ...
    if (this.control.skipping) {
      ...
      setTimeout(
        this.control.nextAction.bind(this),
        time ? time : this.storyConfig.timeout
      );
    }
  }
