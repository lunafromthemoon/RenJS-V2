...
export class RJSMenu extends Group {
  id: string;
  elementFactory: {};
  saveSlots = {};
  ...
  constructor(public game: RJS, public config) {
    super(game);
    this.visible = false;
    this.id = config.id;
  ...
  createImage(element: { x: number; y: number; asset: string }): Sprite {
    const spr: Sprite = this.game.add.sprite(
      element.x,
      element.y,
      element.asset,
      0
  ...
  createButton(element: {
    ...
    this.game.accessibility.button({
      isActive: () =>
        !this.game.control.unskippable &&
        btn.parent === this.game.gui.menus[this.game.gui.currentMenu],
      label: [element.binding, element.menu, element.slot]
        .filter((i) => i)
        .join(' '),
      onclick: () => btn.onClick(),
      onfocus: () => (btn.frame = 1),
  ...
  createSlider(element: {
    ...
    if (element.binding === 'changeUserPreference') {
      const preference =
        this.game.userPreferences.preferences[element.userPreference];
      value =
        (preference.value - preference.min) / (preference.max - preference.min);
    }
    const slider = new MaskSlider(this.game, element, value);
    this.game.accessibility.slider({
      isActive: () =>
        !this.game.control.unskippable &&
        slider.parent === this.game.gui.menus[this.game.gui.currentMenu],
      label: [element.binding, element.userPreference]
        .filter((i) => i)
        .join(' '),
      min: 0,
      max: 1,
  ...
  async show(): Promise<any> {
    ...
    const transition = this.game.screenEffects.transition.get(
      this.game.storyConfig.transitions.menus
    );
